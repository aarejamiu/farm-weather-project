...
document.getElementById('saveBtn')?.addEventListener('click', async () => {
    ...
    if (!w) { alert('Please check weather first before saving.'); return; }
    const note = document.getElementById('note').value;
    try {
        const res = await fetch(`${BASE}/dates/save`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            body: JSON.stringify({ note, location: w.city, temp: w.temp, humidity: w.humidity, wind: w.wind, condition: w.condition })
        });
        if (await handleUnauthorized(res)) return;
        const data = await res.json();
        alert(data.message);
                ...
                <div class="sr-top">
                    ...
                    <div class="sr-info">
                        <div class="sr-date-row">
                            <span class="sr-date">${formatDate(item.createdAt)}</span>
